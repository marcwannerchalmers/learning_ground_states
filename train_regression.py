...
from util.util_lewis import get_feature_vectors, get_omega, get_nearby_qubit_pairs, get_all_edges
from util.data import get_data
from util.jax_reg import fit_best_alpha_jax, final_fit
...
def fit_best_alpha(hp, ML_method, Xfeature_train, Xfeature_test, 
    ...
    for alpha in [2**(-8), 2**(-7), 2**(-6), 2**(-5)]:
        ...
        if best_cv_score > score:
            ...
            train_score = np.linalg.norm(clf.predict(Xfeature_train).ravel() -
                                            y_train.ravel()) / (len(y_train) ** 0.5)
            best_cv_score = score

    return train_score, test_score, best_cv_score
...
def train_and_predict(q1, q2, hp, data, omega):
    ...
    best_cv_score, test_score, train_score = 999.0, 999.0, 999.0
    ...
    # final fit
    Xfeature_train = get_feature_vectors(
            hp.delta1, best_R, X_train, omega, best_gamma, nrow=hp.nrow, ncol=hp.ncol)
    Xfeature_test = get_feature_vectors(
...
def get_new_data(shape, seq=None):
    fname = "{}x{}.td".format(*shape) if seq is None else "{}_{}x{}.td".format(seq, *shape)
    path = "data_torch/"
    data: TensorDict = torch.load(os.path.join(path, fname))
    # subtract 1 s.t. X in [-1, 1]
    X = (data["X"].float() - 1).numpy()
    Y = data["Y"].float().numpy()
    return X, Y, Y


def main(hp, seq=None):
    ...
    """data = get_data(nrow=hp.nrow, ncol=hp.ncol, shadow_size=hp.shadow_size,
                    data_name=hp.data_name, normalize=True, verbose=True)"""
    data = get_new_data((hp.nrow, hp.ncol), seq="lds")
    ...
    data_path = "results2/rg_{}".format(seq)
    result_path = "{}/rg_results_{}_{}x{}_split_{}_delta1_{}.txt".format(data_path, seq, hp.nrow, hp.ncol, split, hp.delta1)
